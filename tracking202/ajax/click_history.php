...
//if spy is enabled, run the query in a certain way.
	if ($_GET['spy'] == 1) {
		...
		$query = query($command, $db_table, false, true, true, ' ORDER BY 202_clicks_spy.click_id DESC ', false, 20, true);
	} else {
...
//now display all the clicks
	...
	<? if (($query['pages'] > 2) and ($_GET['spy'] != 1)) { ?>
		<div class="offset">   <?
			if ($query['offset'] > 0) {
				printf(' <a class="onclick_color" onclick="loadContent(\'click_history.php\',\'%s\');">First</a> ', $i);
				printf(' <a class="onclick_color" onclick="loadContent(\'click_history.php\',\'%s\');">Prev</a> ', $query['offset'] - 1);
			}
			...
			if ($query['pages'] > 1) {
				for ($i=0; $i < $query['pages']-1; $i++) {                         
					if (($i >= $query['offset'] - 10) and ($i < $query['offset'] + 11)) { 
						...
						printf(' <a %s onclick="loadContent(\'click_history.php\',\'%s\');">%s</a> ', $class, $i, $i+1);
						unset($class);
					}
				}
			} 
			...
			if ($query['offset'] < $query['pages'] - 2) {
				printf(' <a class="onclick_color" onclick="loadContent(\'click_history.php\',\'%s\');"">Next</a> ', $query['offset'] + 1);
				printf(' <a class="onclick_color" onclick="loadContent(\'click_history.php\',\'%s\');">Last</a> ', $query['pages'] - 2); 
			} ?>
		</div>   
	<? }   
