...
<? include_once($_SERVER['DOCUMENT_ROOT'] . '/export202/toolbar.php');
	...
	$export_session_result = _mysql_query($export_session_sql); //; // or record_mysql_error($export_session_sql);
	...
	////$csv_file = fopen('/var/www/'.$_SERVER['SERVER_NAME'].'/tmp/'.$export_session_id_public.'-yahoo.csv', 'w');
	...
	$line = array('Campaign Name','Ad Group Name','Component Type','Component Status','Keyword','Keyword Alt Text','Keyword Custom URL','Sponsored Search Bid (USD)','Sponsored Search Bid Limit (USD)','Sponsored Search Status','Match Type','Content Match Bid (USD)','Content Match Bid Limit (USD)','Content Match Status','Ad Name','Ad Title','Ad Short Description','Ad Long Description','Display URL','Destination URL','Watch List','Campaign ID','Campaign Description','Campaign Start Date','Campaign End Date','Ad Group ID','Ad Group: Optimize Ad Display','Ad ID','Keyword ID','Checksum','Error Message');
	...
	//fclose($csv_file);
		...
		?></textarea>
			...
			<? /*<td class="csv-export">
				<? printf('<a href="http://%s/tmp/%s-yahoo.csv"><img src="http://%s/images/excel-48.gif"/></a>',$_SERVER['SERVER_NAME'], $export_session_id_public,$_SERVER['STATIC_SERVER_NAME']); ?>
			</td>*/ ?>
		</tr> 
	</table>
	 	...
	 	<tr>
	 		<td colspan="2">
	 			...
	 			<div>
		 			 ...
		 			 <p>Copy this text and paste it into excell, then save the file as an .XLS file. You should now be able to import this file into your MSN AdCenter account.
	 			</div>
	...
	//$csv_file = fopen('/var/www/'.$_SERVER['SERVER_NAME'].'/tmp/'.$export_session_id_public.'-msn.csv', 'w');
	...
	$line = array('Campaign','Ad Group','Keyword','Keyword Matching','Current Maximum CPC','Keyword Destination URL','Headline','Description Line 1','Description Line 2','Display URL','Destination URL');
	//fputcsv(//$csv_file, $line);
	...
	?>			</textarea>
			...
			<? /*<td class="csv-export">
				<? printf('<a href="http://%s/tmp/%s-msn.csv"><img src="http://%s/images/excel-48.gif"/></a>',$_SERVER['SERVER_NAME'], $export_session_id_public,$_SERVER['STATIC_SERVER_NAME']); ?>
			</td>*/ ?>
		</tr>
	</table>
	...
	//$csv_file = fopen('/var/www/'.$_SERVER['SERVER_NAME'].'/tmp/'.$export_session_id_public.'-google-adgroups.csv', 'w');
	...
	?>			</textarea>
			...
			<? /*<td class="csv-export">
				<? printf('<a href="http://%s/tmp/%s-google-adgroups.csv"><img src="http://%s/images/excel-48.gif"/></a>',$_SERVER['SERVER_NAME'], $export_session_id_public,$_SERVER['STATIC_SERVER_NAME']); ?>
			</td> */ ?>
		</tr>
	</table>
