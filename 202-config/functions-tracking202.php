  ...
  function display_calendar($page, $show_time, $show_adv, $show_bottom, $show_limit, $show_breakdown, $show_type, $show_cpc_or_cpv = true) { 
							...
							<table cellspacing="0" cellpadding="0" class="s-table-left" <? if ($show_bottom == false) { echo 'style="display:none;"'; } ?>>
								<tr>
									...
									<td><select name="user_pref_show" <? if ($show_type == false) { echo 'style="display:none;"'; } ?>>
										...
										</select><? if ($show_type == false) { ?><input type="text" readonly="readonly" value="Show Real Clicks" /><? } ?></td>
						...
						<td>
							<table cellspacing="0" cellpadding="0" class="s-table-right">
								<tr >
									...
									<? if ($navigation[1] == 'tracking202') { ?><td><input type="submit" id="s-search" class="s-submit s-submit1" onclick="set_user_prefs('<? echo $html['page']; ?>');" value="Set Preferences"/></td><? } 
									 elseif ($navigation[1] =='tracking202api') { ?><td><input type="submit" id="s-search" class="s-submit s-submit1" onclick="set_user_prefs2('<? echo $html['page']; ?>');" value="Set Preferences"/></td> <? } ?>
								</tr>
							</table>
						</td>
					</tr>
   ...
   <? /*<div style="position: relative;">
	<img id="m-loader" style="z-index: 100;
		...
		top: 45%;
		left: 50%;
		" src="/202-img/loader-big.gif"/> </div>*/ ?>
   
   
   <? if ($navigation[1] == 'tracking202api') { show_tracking202api_nav_bar(); } ?>
...
//this function takes in the user's img file, and make sure it exists in the file system, because we load balance, if its not synced we don't want to display images that cna't be seen because the servers haven't synced yet.
function user_pic_exists($user_pic_file) { 
	
	//if there isn't anything return false right away
	if ($user_pic_file == '') { return false; }
	
	//if this user pic does exist, return true, and display it
	$upload_dir = $_SERVER['DOCUMENT_ROOT']. '/../' . $_SERVER['SERVER_NAME'] . '/users/';
	
	$file = $upload_dir . $user_pic_file;
	if (file_exists($file)) { return true; } else { return false; }
